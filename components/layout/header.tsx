...
const Header = () => {
  ...
  const navigation = [
    ...
    { name: 'Anklets', href: '/products/anklets' },
        ...
        <div className="flex items-center justify-between h-16 lg:h-20">
          ...
          <Link href="/" className="flex items-center space-x-2">
            ...
            <div className="hidden sm:block">
              <h1 className="text-xl lg:text-2xl font-bold" style={{ color: '#0A8E81' }}>
                FaeLight Crafts
              </h1>
              <p className="text-xs text-gray-600">Artisan Jewelry</p>
            </div>
          </Link>
          ...
          <nav className="hidden lg:flex items-center space-x-8">
            {navigation.map((item) => (
              <Link
                key={item.name}
                href={item.href}
                className="text-gray-700 hover:text-[#0A8E81] transition-colors font-medium"
              >
                {item.name}
              </Link>
            ))}
                ...
                <div className="flex flex-col space-y-4 mt-8">
                  {navigation.map((item) => (
                    <Link
                      key={item.name}
                      href={item.href}
                      className="text-lg font-medium text-gray-700 hover:text-[#0A8E81] transition-colors py-2"
                    >
                      {item.name}
                    </Link>
                  ))}
